...
namespace mod_groupquiz\forms\edit;
...
class add_question_form extends \moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('static', 'questionid', get_string('question', 'groupquiz'), $this->_customdata['questionname']);

        $mform->addElement('text', 'points', get_string('points', 'groupquiz'));
        $mform->addRule('points', get_string('invalid_points', 'groupquiz'), 'required', null, 'client');
        $mform->addRule('points', get_string('invalid_points', 'groupquiz'), 'numeric', null, 'client');
        $mform->setType('points', PARAM_FLOAT);
        $mform->setDefault('points', number_format($this->_customdata['defaultmark'], 2));
        $mform->addHelpButton('points', 'points', 'groupquiz');

        if (!empty($this->_customdata['edit'])) {
            $savestring = get_string('savequestion', 'groupquiz');
        } else {
            $savestring = get_string('addquestion', 'groupquiz');
        }
    ...
    public function validation($data, $files) {
        ...
        if (!filter_var($data['points'], FILTER_VALIDATE_FLOAT) && filter_var($data['points'], FILTER_VALIDATE_FLOAT) != 0) {
            $errors['points'] = get_string('invalid_points', 'groupquiz');
